...
def cart2hom(points):
  ...
  """
  points_hom = np.ones((*points.shape[:-1], points.shape[-1] + 1))
...
def gettranslation(v):
  ...
  """
  vector_hom = cart2hom(v)
  T = np.eye(vector_hom.shape[0])
...
def getyrotation(d):
  ...
  Ry = np.matrix([[np.cos(d), 0, np.sin(d)], 
        [0, 1, 0], 
        [-np.sin(d), 0, np.cos(d)]])
  return(Ry)
...
def getzrotation(d):
  ...
  Rz = np.matrix([[np.cos(d), -np.sin(d), 0], 
        [np.sin(d), np.cos(d), 0], 
        [0, 0, 1]])
  return(Rz)
...
def getcentralprojection(principal, focal):
  ...
  L = np.matrix([[focal, 0, principal[0]],
      [0, focal, principal[1]],
      [0, 0, 1]])
