...
def load_faces(path, ext=".pgm"):
    ...
    for pics in files:
        if os.path.splitext(pics)[0] == '1' and os.path.splitext(pics)[1][1:] in ['pgm']:
...
def vectorize_images(imgs):
    ...
    x=imgs.reshape(imgs.shape[0], (imgs.shape[1]*imgs.shape[2]))
...
def compute_pca(X):
    ...
    mean_face = X - np.mean(X.T, axis=1)
    
    #C: covariance matrix
    C = np.cov(mean_face.T)
    
    eigen_values, eigen_vectors = np.linalg(C)
    
    u = eigen_vectors
    cumul_var = eigen_values
